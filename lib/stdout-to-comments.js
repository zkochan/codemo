...
const addHook = require('./hook-console-log').addHook
  ...
  return codeLines.reduce((acc, line, index) => {
    ...
    return {
      ...
      codeLines: acc.codeLines
        .concat(line)
        .concat(outputsToComments(lineOutputs, line)),
    }
  }, {
    codeLines: [],
    outputs: sOutputs,
...
function outputsToComments (lineOutputs, prevLine) {
  if (!lineOutputs.length) return []

  const linePadding = getLinePadding(prevLine)
  return lineOutputs
    .map(lineOutput => lineOutput.args)
    .map(outputArgs => consoleStringify.apply(null, outputArgs))
    .map(output => outputToComment(output, linePadding))
}
